...
public class TimetableManager {
    ...
    public static ArrayList<Period> returnAllFreePeriodsOnCertainDay(
        ...
        for (
            int startHour = FIRST_HOUR;
            startHour <= LAST_HOUR - duration + 1;
            startHour++
        ) {
            boolean isFree = true;
    ...
    public static void implementRandomLunchBreakOnDay(
        ...
        final int HIGHEST_SCHOOLHOUR = timetable
            .getClassSubjectInstances()
            .stream()
            .filter(e -> e.getPeriod().getSchoolDays() == schoolday)
            .mapToInt(e -> e.getPeriod().getSchoolHour())
            .max()
            .getAsInt(); // get highest Schoolhour
        final int LOWEST_SCHOOLHOUR = timetable
            .getClassSubjectInstances()
            .stream()
            .filter(e -> e.getPeriod().getSchoolDays() == schoolday)
            .mapToInt(e -> e.getPeriod().getSchoolHour())
            .min()
            .getAsInt(); // get lowest Schoolhour

        final Random random = new Random();

        final int randBob = random.nextInt(
            LOWEST_SCHOOLHOUR + 2,
            HIGHEST_SCHOOLHOUR + 1
        );
        ...
        timetable
            ...
            .filter(
                e ->
                    ...
                    e.getPeriod().getSchoolHour() >= randBob
            )
            .forEach(e ->
                e.getPeriod().setSchoolHour(e.getPeriod().getSchoolHour() + 1)
            ); // move each class after

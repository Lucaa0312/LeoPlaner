...
function initSubjectSearch() {
    ...
    input.addEventListener("input", () => {
        ...
        if (!query)
            return;
        const matches = allSubjects.filter(s => s.subjectName.toLowerCase().includes(query) &&
...
function addTeacher() {
    ...
    if (!firstNameInput || !lastNameInput || !initialsInput)
        return;
    const teacherData = {
    ...
    fetch("http://localhost:8080/api/teachers")
        ...
        if (!data || data.length === 0) {
            ...
            if (noTeachersElement)
                noTeachersElement.style.display = "block";
            return;
        }
        else {
            const noTeachersElement = document.getElementById("no-teachers");
            ...
            data.forEach((teacher) => {
                ...
                const subjectsHTML = `
                        ${visibleSubjects.map((s) => `<span class="subject-chip">${s.subjectName}</span>`).join("")}
...
function searchTeacher() {
    ...
    rows.forEach(function (row) {
        ...
        if (!nameElement || !initialsElement || !subjectsElement)
            return;

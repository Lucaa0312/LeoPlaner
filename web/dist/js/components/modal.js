...
let titleCounter = 0;
export function buildModal(root, options) {
    root.classList.add("modal");
    root.classList.remove("modal--sm", "modal--lg");
    if (options.size === "sm")
        root.classList.add("modal--sm");
    if (options.size === "lg")
        root.classList.add("modal--lg");
    ...
    if (options.onClose)
        closeCallbacks.set(root, options.onClose);
    else
        closeCallbacks.delete(root);
    return { root, panel, header, heading, aside, body, footer, setTitle };
}
export function openModal(root, options) {
    if (openState.has(root))
        return;
    const panel = root.querySelector(".modal__panel") ?? root;
    ...
    window.requestAnimationFrame(() => {
        const items = focusables(panel).filter((el) => !el.classList.contains("modal__close"));
        (items[0] ?? panel).focus();
    });
}

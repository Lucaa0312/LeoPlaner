...
async function silentGet(path, fallback) {
    try {
...
async function loadSnapshot() {
    ...
    const [teachers, subjects, rooms, classSubjects, classes, algorithmRunning, algorithmRanOnce, history,] = await Promise.all([
        silentGet("/teachers/withWishes", []),
        silentGet("/subjects", []),
        silentGet("/rooms", []),
        silentGet("/classSubjects", []),
        silentGet("/getAllClasses", []),
        silentGet("/isAlgorithmRunning", false),
        silentGet("/isAlgorithmRunningAtLeastOnce", false),
        silentGet("/get/algorithmHistory", []),
    ]);
    return {
...
function teacherLoads(snapshot) {
    const hoursById = new Map();
    for (const item of snapshot.classSubjects) {
        for (const teacher of item.teacher ?? []) {
            hoursById.set(teacher.id, (hoursById.get(teacher.id) ?? 0) + item.weeklyHours);
        }
    }
    return snapshot.teachers
        ...
        availableHours: Math.max(0, SLOTS_PER_WEEK - (teacher.teacherNonWorkingHours?.length ?? 0)),
    }))
...
function requiredTypesOf(snapshot, item) {
    const own = item.subject?.requiredRoomTypes;
    if (own && own.length > 0)
        return own;
    const full = snapshot.subjects.find((subject) => subject.id === item.subject?.id);
...
function roomDemands(snapshot) {
    ...
    for (const item of snapshot.classSubjects) {
        ...
        const targets = types.length > 0 ? types : ["CLASSROOM"];
...
function missingRoomTypes(snapshot) {
    ...
    for (const subject of snapshot.subjects) {
        for (const type of subject.requiredRoomTypes ?? [])
            required.add(type);
    }
    return [...required].filter((type) => !available.has(type));
}
function totalWeeklyHours(snapshot) {
    return snapshot.classSubjects.reduce((sum, item) => sum + item.weeklyHours, 0);
}
function isEmpty(snapshot) {
    return (snapshot.teachers.length === 0 &&
...
function panelHead(title, caption, link) {
    const head = el("div", "panel__head");
    const text = el("div", "panel__heading");
    text.appendChild(el("h2", "panel__title", title));
    if (caption)
        text.appendChild(el("p", "panel__caption", caption));
...
function barRow(options) {
    ...
    if (options.sub)
        row.appendChild(el("span", "bar-row__sub", options.sub));
    return row;
}
...
function renderActions(snapshot) {
    ...
    host.append(input, importButton, exportButton, timetable, status, importError, exportError);
    initImportButton();
    initExportButton();
}
// ------------------------------------------------------------------- notices
function renderNotice(snapshot) {
    ...
    if (!snapshot.reachable) {
        ...
        host.append(icon("ti ti-plug-connected-x"), el("div", "notice__text", "Das Backend ist nicht erreichbar. Die angezeigten Zahlen sind leer, bis der Server unter localhost:8080 läuft."));
        return;
    }
    if (isEmpty(snapshot)) {
        host.hidden = false;
        ...
        text.append(el("strong", undefined, "Noch keine Daten vorhanden. "), "Importieren Sie eine Excel-Datei oder legen Sie Lehrer, Fächer, Räume und Klassen über die Seitenleiste an.");
...
function buildChecks(snapshot) {
    ...
    const classesWithoutSubjects = loads.filter((load) => load.subjectCount === 0);
    const classesWithoutRoom = loads.filter((load) => !load.hasRoom);
    const lessonsWithoutTeacher = snapshot.classSubjects.filter((item) => (item.teacher?.length ?? 0) === 0);
    ...
    return [
        {
            ...
            detail: snapshot.teachers.length > 0
        ...
        },
        {
            ...
            detail: snapshot.rooms.length > 0
        ...
        },
        {
            ...
            detail: classesWithoutSubjects.length === 0
        ...
        },
        {
            ...
            detail: lessonsWithoutTeacher.length === 0
        ...
        },
        {
            ...
            detail: orphanSubjects.length === 0
        ...
        },
        {
            ...
            detail: overloadedTypes.length === 0
        ...
        },
        {
            ...
            detail: classesWithoutRoom.length === 0
...
function readinessRing(passed, total, tone) {
...
function renderReadiness(snapshot) {
    ...
    panel.appendChild(panelHead("Planungsbereitschaft", blocked > 0
    ...
    checks.forEach((check) => {
        ...
        text.append(el("span", "check__label", check.label), el("span", "check__detail", check.detail));
...
function metric(label, value, sub) {
    ...
    if (sub)
        box.appendChild(el("span", "metric__sub", sub));
    return box;
}
function renderPlanning(snapshot) {
    ...
    const best = snapshot.history.reduce((min, entry) => (min === null || entry.cost < min ? entry.cost : min), null);
    const scheduled = snapshot.classes.filter((schoolClass) => schoolClass.timetable !== null).length;
    ...
    panel.appendChild(panelHead("Stundenplanung", "Stand des Optimierers", {
        href: "timetable.html",
    ...
    grid.append(metric("Zu verplanen", formatNumber(totalWeeklyHours(snapshot)), "Wochenstunden"), metric("Klassen mit Plan", `${formatNumber(scheduled)}/${formatNumber(snapshot.classes.length)}`, "Stundenpläne angelegt"), metric("Aktuelle Kosten", last ? formatNumber(last.cost) : "–", best !== null ? `Bester Wert: ${formatNumber(best)}` : "Noch kein Lauf"), metric("Iterationen", last ? formatNumber(last.iteration) : "–", `${plural(snapshot.history.length, "Messpunkt", "Messpunkte")}`));
    panel.appendChild(grid);
    panel.appendChild(emptyHint(state === "idle"
...
// ------------------------------------------------------------- class workload
function renderClassHours(snapshot) {
    ...
    panel.appendChild(panelHead("Wochenstunden je Klasse", loads.length === 0
        ? "Noch keine Klassen angelegt"
        : `Ø ${formatNumber(average)} h · ${plural(total, "Stunde", "Stunden")} gesamt`, { href: "classSubjects.html", label: "Klassen" }));
    if (loads.length === 0) {
        panel.appendChild(emptyHint("Sobald Klassen mit Fächern vorhanden sind, zeigt diese Liste die Stundenverteilung."));
        return;
    }
...
// ------------------------------------------------------------- teacher load
function renderTeacherLoad(snapshot) {
    ...
    const average = assigned.length === 0
        ? 0
        : Math.round(assigned.reduce((sum, load) => sum + load.weeklyHours, 0) /
            assigned.length);
    panel.appendChild(panelHead("Lehrerauslastung", loads.length === 0
        ? "Noch keine Lehrkräfte angelegt"
        : `Ø ${formatNumber(average)} h je eingeteilter Lehrkraft`, { href: "teacher.html", label: "Lehrer" }));
    if (loads.length === 0) {
        panel.appendChild(emptyHint("Die Auslastung ergibt sich aus den Wochenstunden der zugeordneten Unterrichtseinheiten."));
        return;
    }
    const list = el("ul", "bar-list");
    loads.forEach((load) => {
        const ratio = load.availableHours === 0 ? 1 : load.weeklyHours / load.availableHours;
        list.appendChild(barRow({
            label: `${load.name}${load.symbol ? ` (${load.symbol})` : ""}`,
            value: `${formatNumber(load.weeklyHours)} h`,
            sub: load.weeklyHours === 0
                ? "Keine Einheiten zugeordnet"
                : `${Math.round(ratio * 100)} % von ${formatNumber(load.availableHours)} verfügbaren Stunden`,
            ratio,
            tone: load.weeklyHours === 0 ? "danger" : ratio > 0.8 ? "accent" : "primary",
        }));
    });
    panel.appendChild(list);
    panel.appendChild(emptyHint("Einheiten mit mehreren Lehrkräften zählen für jede von ihnen. Verfügbare Stunden = 50 Einheiten pro Woche abzüglich gesperrter Zeiten."));
}
// -------------------------------------------------------------- room demand
function renderRoomDemand(snapshot) {
    const panel = document.getElementById("room-demand-panel");
    if (!panel)
        return;
    panel.replaceChildren();
    const demands = roomDemands(snapshot);
    panel.appendChild(panelHead("Räume & Raumbedarf", "Geforderte Wochenstunden gegen die vorhandenen Plätze – Einheiten ohne Fachraumbedarf zählen zum Klassenraum", { href: "rooms.html", label: "Räume" }));
    if (demands.length === 0) {
        panel.appendChild(emptyHint("Noch keine Räume angelegt und keine Fachräume gefordert."));
        return;
    }
    ...
    demands.forEach((entry) => {
        const ratio = entry.capacityHours === 0
            ? entry.demandHours > 0
                ? 1
                : 0
            : entry.demandHours / entry.capacityHours;
        const tone = ratio > 1 || (entry.capacityHours === 0 && entry.demandHours > 0)
            ? "danger"
            : ratio > 0.8
                ? "accent"
                : "success";
        ...
        const capacity = el("td", "is-num", `${formatNumber(entry.capacityHours)} h`);
        ...
        track.appendChild(fill);
        usage.append(track, el("span", "room-demand__value", entry.capacityHours === 0
            ? "kein Raum"
    ...
    const overloaded = demands.filter((entry) => entry.demandHours > entry.capacityHours);
    panel.appendChild(emptyHint(overloaded.length === 0

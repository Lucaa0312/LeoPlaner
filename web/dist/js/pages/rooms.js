...
async function loadAndRenderRooms() {
    ...
    if (firstLoad)
        renderSkeleton(roomsContainer, 8, "card");
    try {
...
function buildField(id, label, placeholder, options = {}) {
...
function buildAddRoomModalContent() {
    ...
    nameInitialsDiv.append(buildField("name-input", "Name", "z. B. Physiksaal", { required: true }), buildField("initials-input", "Abkürzung", "z. B. PH1", {
        required: true,
    ...
    numberPrefixSuffixDiv.append(buildField("number-input", "Nummer", "z. B. 204", { inputMode: "numeric" }), buildField("prefix-input", "Präfix", "z. B. E", { hint: "Optional" }), buildField("suffix-input", "Suffix", "z. B. a", { hint: "Optional" }));
...
function openRoomForm(room) {
    ...
    const prefixInput = content.querySelector("#prefix-input");
    const suffixInput = content.querySelector("#suffix-input");
    if (room) {
        ...
        if (prefixInput)
            prefixInput.value = room.roomPrefix ?? "";
        if (suffixInput)
            suffixInput.value = room.roomSuffix ?? "";
    }

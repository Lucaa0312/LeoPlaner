...
document.addEventListener("click", (event: MouseEvent) => {
  ...
  document
    .querySelectorAll<HTMLElement>(".top-bar-select")
    .forEach((wrapper) => {
...
function createLayout(data: ClassSubjectInstance[]): void {
  ...
  map.forEach((classSubjectInstances, day) => {
    ...
    classSubjectInstances.forEach((item) => {
      ...
      const subjectName =
        item.classSubject?.subject?.subjectName || "No lesson";
      ...
      const subjectColorRed =
        item.classSubject?.subject?.subjectColor?.red || 200;
      const subjectColorGreen =
        item.classSubject?.subject?.subjectColor?.green || 200;
      const subjectColorBlue =
        item.classSubject?.subject?.subjectColor?.blue || 200;
      ...
      if (
        ...
        if (
          (!alreadyAddedBreak && duration >= 3 && period == 1) ||
          (duration >= 3 && period == 2) ||
          (duration >= 2 && period == 2)
        ) {
          height =
